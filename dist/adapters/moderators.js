 ...
 */
export class RoundRobinModerator {
    port = 'round-robin';
    minSpeeches;
    minRounds;
    constructor(options = {}) {
    ...
    async decide(input) {
        ...
        if (quiet.length > 0) {
            return { action: 'invite', next: quiet[0], note: '本轮还没发言' };
        }
        // 本轮都说过话了 → 轮转到累计发言最少的人。
        ...
        return { action: 'invite', next: sorted[0], note: '本轮已轮转，继续均衡发言' };
    }
}
/**
 * 脚本化主持人：按预设队列给出决定。
 ...
 */
export class ScriptedModerator {
    port = 'scripted-moderator';
    decisions;
    fallback;
    /** 记录每次拿到的上下文，便于断言"主持人到底看到了什么"。 */
    seen = [];
    constructor(decisions, fallback) {
