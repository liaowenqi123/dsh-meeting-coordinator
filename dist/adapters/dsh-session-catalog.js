...
export function readSessionTitle(ctx, session) {
    const titles = readService(ctx, 'sessionTitle');
...
function truncateByCodePoints(input, maxChars) {
    const points = [...input];
    return points.length <= maxChars ? input : `${points.slice(0, maxChars - 1).join('')}…`;
}
/** 用第一条人类消息派生回退标题。 */
function fallbackTitleFromMessages(session) {
 ...
 * 所以宁严勿松。
 */
export function listSessionCandidates(options) {
    // --- 来源一：已加载的会话（有 header / deriveMessages，信息最权威）---------
    const live = new Map();
    const store = readService(options.ctx, 'sessions');
...
function readPersistedTitle(row) {
    ...
    if (typeof raw === 'object' && raw !== null) {
        const nested = raw.title;
        if (typeof nested === 'string')
            return normalizeTitle(nested);
    }
    return undefined;
}
/**
...
export function readSessionContext(options) {
    ...
    let session;
    try {
        session = store.get(options.sessionId);
    }
    catch {
    ...
    try {
        if (typeof session.deriveMessages === 'function') {
            ...
            const conversational = [...messages].filter((message) => {
                ...
                if (isBoilerplateUserMessage(record))
                    return false;
                return true;
            });
 ...
 */
export function isBoilerplateUserMessage(message) {
    const record = message;
    const body = typeof record?.content === 'string'
        ? record.content
        : Array.isArray(record?.content)
    ...
    if (head.length === 0)
        return true;
    return (head.startsWith('Current runtime context') ||
...
//# sourceMappingURL=dsh-session-catalog.js.map