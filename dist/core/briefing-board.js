...
import { assertDraftWellFormed, assertWithinBudget, briefingFingerprint, countChars, normalizeField, } from './briefing-text.js';
export class BoardCorrupted extends Error {
    code = 'meeting/board-corrupted';
    constructor(message) {
        super(message);
...
export class BriefingBoard {
    ...
    summarize(input) {
        ...
        const header = `[例会] board=${this.boardDomain} scope=${input.scope} trigger=${input.trigger} ` +
        ...
        for (const item of items) {
            ...
            if (item.blocker !== null)
                parts.push(`障碍:${item.blocker}`);
            if (item.needs.length > 0)
                parts.push(`需要:${item.needs.join('/')}`);
...
function coerceRecord(value, filePath) {
...
//# sourceMappingURL=briefing-board.js.map