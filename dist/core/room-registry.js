...
export class RoomRegistry {
    ...
    join(input) {
        ...
        if (existing !== undefined && existing !== input.roomId) {
            throw new RoomRegistryError(`会话 ${input.sessionId} 已属于会议室 ${existing}；一个会话只能属于一个会议室，请先 leave()。`);
        }
        const room = this.requireRoom(input.roomId);
        if (room.members.some((member) => member.sessionId === input.sessionId))
            return room;
        const event = {
            v: 1,
            kind: 'member-joined',
    ...
    leave(sessionId) {
        this.ensureLoaded();
        if (!this.membership.has(sessionId))
            return;
        const event = { v: 1, kind: 'member-left', sessionId, at: this.now() };
        this.append(event);
        this.apply(event);
    }
    // --- 读 -----------------------------------------------------------------
    ...
    requireRoom(id) {
        const room = this.rooms.get(id);
        if (room === undefined)
            throw new RoomRegistryError(`会议室 ${id} 不存在。`);
...
function freezeRoom(room) {
    return Object.freeze({
        ...
        members: Object.freeze([...room.members.values()].sort((left, right) => (left.sessionId < right.sessionId ? -1 : left.sessionId > right.sessionId ? 1 : 0))),
    });
}
//# sourceMappingURL=room-registry.js.map