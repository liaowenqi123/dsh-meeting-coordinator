...
export const DEFAULT_ROOM_POLICY = {
...
export class MeetingRoom {
    ...
    nextRound() {
        if (this.closed)
            throw new MeetingRoomError(`会议 ${this.id} 已结束。`);
        this.currentRound += 1;
    ...
    spokeThisRound() {
        ...
        for (const turn of this.turns) {
            if (turn.round === this.currentRound && turn.kind === 'speech')
    ...
    transcriptProjection() {
        ...
        for (let index = rendered.length - 1; index >= 0; index -= 1) {
            ...
            if (used + cost > this.policy.transcriptBudgetChars)
                break;
            kept.unshift(line);
            used += cost;
        }
    ...
    push(round, speaker, kind, text, cap) {
        ...
        this.sequence += 1;
        const turn = { seq: this.sequence, round, speaker, kind, text: trimmed, chars, at: this.now() };
