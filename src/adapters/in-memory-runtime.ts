...
interface InMemoryAgent {
  readonly id: string
  readonly spec: AgentSlotSpec
  readonly context: string[]
  state: AgentRunState
  closed: boolean
  interrupted: number
  wakeCount: number
  /** 用 steer 语义发给空闲成员的条数：上游会排队，但不会唤醒它。 */
  steerWhileIdle: number
 ...
 * 而不是架构宣言。
 */
export class InMemoryAgentRuntime implements AgentRuntimePort {
  readonly port = 'in-memory'
  private readonly agents = new Map<AgentSlotId, InMemoryAgent>()
  private readonly options: InMemoryRuntimeOptions
  /** 记录每一次投递，用于断言"到底传了什么过去、用什么语义传的"。 */
  readonly deliveries: { readonly slot: AgentSlotId; readonly delivery: Delivery }[] = []
  ...
  async waitForActivity(
    ...
    return await new Promise<ActivityObservation>((resolve) => {
      let settled = false
      const settle = (observation: ActivityObservation): void => {
        if (settled) return
        settled = true
        clearTimeout(timer)
      ...
      const onActivity = (): void => settle({ kind: 'changed', detail: `槽位 ${handle.slot} 产生了新活动。` })
  ...
  private notifyWaiters(agent: InMemoryAgent): void {
    const waiters = [...agent.waiters]
    for (const waiter of waiters) waiter()
  }

  private mustGet(handle: AgentHandle): InMemoryAgent {
  ...
  private mustGetBySlot(slot: AgentSlotId): InMemoryAgent {
