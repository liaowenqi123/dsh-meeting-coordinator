...
function buildService(console: MeetingConsole): object {
  ...
  Object.defineProperty(prototype, REMOTE_METHOD_DESCRIPTOR, {
    value: Object.freeze({
      ...
      methods: Object.freeze(
        REMOTE_METHODS.map((method) => Object.freeze({ method, invocation: Object.freeze({ kind: 'direct' }) })),
      ),
    }),
    writable: false,
    enumerable: false,
  ...
  const service = Object.create(prototype) as Record<string, unknown>
 ...
 */
export function registerMeetingRemote(options: RegisterMeetingRemoteOptions): MeetingRemoteHandle {
  const notes: string[] = []
  const ctx = options.ctx as DshRemoteContextFace | undefined

  const base = {
    serviceKey: MEETING_REMOTE_SERVICE,
    namespace: MEETING_REMOTE_NAMESPACE,
    methods: [...REMOTE_METHODS] as readonly string[],
  }
  ...
  try {
    ...
    } else if (returned !== undefined && returned !== null && typeof (returned as Promise<unknown>).then === 'function') {
