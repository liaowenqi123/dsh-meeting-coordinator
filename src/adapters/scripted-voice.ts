 ...
 */
function defaultReflect(participant: string, transcript: string, maxChars: number): string {
  const scored: { score: number; line: string }[] = []
  for (const line of transcript.split('\n')) {
    if (!line.startsWith('[R')) continue
    const body = line.replace(/^\[R\d+\]\s*/, '')
    const speaker = body.split(':')[0]?.trim() ?? ''
    if (speaker === participant) continue
  ...
  if (scored.length === 0) return '本次会议无与我相关的事项。'
  ...
  if (kept.length === 0) {
    const first = scored[0] as { line: string }
    return first.line.slice(0, Math.max(1, maxChars - 1)) + '…'
  }
  return kept.join(' ')
}

export class ScriptedMeetingVoice implements MeetingVoicePort {
  readonly port = 'scripted'
  private readonly scripts: Readonly<Record<string, ScriptedParticipantScript>>
  ...
  async speak(request: MeetingVoiceRequest): Promise<string> {
    ...
    if (request.purpose === 'reflect') {
      ...
      if (custom !== undefined) return custom(extractTranscript(request.prompt))
    ...
    if (next === undefined) {
      throw new VoiceUnavailable(
        `${request.participant} 没有更多的发言脚本（已消费 ${cursor} 条）。若会议需要它再次发言，请补足脚本。`,
      )
    }
