...
import { compose, CompositionRuleCatalog, type CompositionPlan } from '@dsh-std/composition'
import {
...
import { parseManifest, projectManifest, type ComponentFacet, type ComponentManifest } from '@dsh-std/manifest'
...
import { RoomMinutesLog, type RoomMinutesStore } from './core/room-minutes.js'
import type { AgentRuntimePort } from './ports/agent-runtime.js'
import { RuntimeUnavailable } from './ports/agent-runtime.js'
import { createDshMeetingRuntime, readService, resolveLiveAgent, type DshContextFace } from './adapters/dsh-team-runtime.js'
import {
...
import { attachDshSessionState, type DshSessionStateHandle, type SessionActivityKind } from './adapters/dsh-session-state.js'
...
import { HUMAN_PARTICIPANT, type AgentParticipant } from './core/participant.js'
 ...
 */
function resolveLogger(ctx: DshPluginContextFace): DshPluginContextFace['logger'] {
  try {
  ...
  return viaGet === undefined || viaGet === null
    ? undefined
    : (viaGet as DshPluginContextFace['logger'])
}
...
export function selectRoomInvitees(input: {
  ...
  for (const signal of input.signals) {
    for (const slot of signal.slots) chosen.add(slot)
  }
  for (const briefing of input.briefings) {
    if (briefing.blocker !== null || briefing.needs.length > 0 || briefing.requestedFrom.length > 0) {
      ...
      for (const target of briefing.requestedFrom) chosen.add(target)
    }
  }
 ...
 */
export function pickEscalationRoom(
  registry: RoomRegistry,
  stalledSlots: readonly string[],
 ...
 */
export async function activateMeetingHost(options: MeetingHostOptions): Promise<MeetingHostHandle> {
  ...
  const providerFacet: ComponentFacet = {
  ...
  const providerManifest: ComponentManifest = {
    ...projected,
    spec: { ...projected.spec, facets: [providerFacet] },
  }

  const consumerManifests: ComponentManifest[] = options.slots.map((slot) => ({
  ...
  if (!plan.compatible) {
    ...
    throw new RuntimeUnavailable(
      `组合预检未通过：${errors.map((issue) => `${issue.code}@${issue.path}: ${issue.message}`).join('; ')}`,
    )
  }
  const unexpectedSkips = plan.skipped.filter((row) => row.code === 'activation-unavailable')
  if (unexpectedSkips.length > 0) {
    throw new RuntimeUnavailable(
      `以下 facet 因缺少 activation driver 被跳过：${unexpectedSkips.map((row) => row.message).join('; ')}`,
    )
  }

  // --- 4. facet 与 driver -------------------------------------------------
  let shared: MeetingSharedState | undefined
  ...
  const agentFacets = options.slots.map((slot) =>
    createAgentFacet({
      boardDomain,
      rootDir: options.rootDir,
      slot,
      ...
      now: options.now,
    } as unknown as Parameters<typeof createAgentFacet>[0]),
  )
  ...
  const driver: ActivationDriver = {
    ...
    async activate(request) {
      ...
      if (name === COORDINATOR_FACET) {
        ...
        if (shared === undefined) throw new RuntimeUnavailable('协调器 facet 激活后没有产出共享状态。')
        return
      }
  ...
  let coordinator: MeetingCoordinator | undefined
  options.onPlan?.(plan)
  try {
    const handles = await lifecycle.activate(plan)
    if (shared === undefined) throw new RuntimeUnavailable('激活完成但没有共享状态。')
     ...
     */
    const ensureRoster = (): void => {
      const titles = new Map<string, string>()
      for (const candidate of options.candidates?.list() ?? []) {
        if (candidate.title !== undefined) titles.set(candidate.sessionId, candidate.title)
      }
      for (const room of rooms.list()) {
        for (const member of room.members) {
          ...
          if (orchestrator.participantIds.includes(member.sessionId)) {
            // 已在册：只补标题（建名册时可能还没探到）。
            if (title !== undefined) orchestrator.member(member.sessionId).retitle(title)
            continue
          }
          orchestrator.enroll({
            id: member.sessionId,
          ...
          if (participant.state === 'working') participant.beginIdleWaiting()
        }
      }
    }
    ...
    const console = new MeetingConsole({
      ...
      runtime: {
        ...
        enroll: (spec) => {
          if (orchestrator.participantIds.includes(spec.id)) return
          orchestrator.enroll({
            id: spec.id,
            domain: spec.domain,
            title: spec.title,
          ...
          if (member.state === 'working') member.beginIdleWaiting()
        },
    ...
    const retryMembers = async (): Promise<readonly string[]> => {
      ...
      if (recovered.length > 0) await options.onMembersSpawned?.()
      return recovered
    }
     ...
     */
    const escalate = async (evaluation: TriggerEvaluation): Promise<RoomEscalationResult> => {
      ...
      const target = pickEscalationRoom(
        rooms,
        evaluation.stallSignals.flatMap((signal) => [...signal.slots]),
      )
      if (target === undefined) {
        return {
          escalated: false,
      ...
      try {
        ...
        return {
          escalated: true,
          reason:
            `停滞升级成立（${invitees.length === 0 ? '大会' : `小会：${invitees.join('、')}`}）；` +
    ...
    return {
      ...
      get pendingMembers(): readonly string[] {
        const state = shared
        if (state === undefined) return options.slots.map((slot) => slot.id)
      ...
      shutdown: async () => {
        ...
        if (orchestrator.activeRoom() !== undefined) {
          for (const id of orchestrator.participantIds) {
            ...
            if (member.isInMeeting() || member.isAwaitingEntry()) member.dismiss()
          }
        }
    ...
    throw new RuntimeUnavailable(
      `facet 激活失败，已由 lifecycle 回滚：${error instanceof Error ? error.message : String(error)}`,
    )
  }
}

// ---------------------------------------------------------------------------
// Cordis 插件入口（真实 DSH 装载路径）
// ---------------------------------------------------------------------------
...
export function renderBriefingInstruction(maxChars: number): string {
  return [
    '## 简报契约',
 ...
 */
export function readSlotsFromEnv(env: Record<string, string | undefined>): readonly AgentSlotSpec[] {
  ...
  try {
    ...
    throw new RuntimeUnavailable(
      `DSH_MEETING_SLOTS 不是合法 JSON：${error instanceof Error ? error.message : String(error)}`,
    )
  }
  ...
  return parsed.map((item, index) => {
    ...
    return record as unknown as AgentSlotSpec
  })
}
 ...
 */
export async function apply(ctx: DshPluginContextFace, config: MeetingPluginConfig = {}): Promise<MeetingPluginActivation> {
   ...
   */
  const gapsAtLoad: readonly string[] = [...runtimeGap(), ...voiceGap()]
  ...
  let rosterOf: () => readonly string[] = () => []
  const resolveMemberId = (sessionId: string): string | undefined => {
    const roster = rosterOf()
    const mapped = sessionIndex.get(sessionId)
    if (mapped !== undefined) return roster.includes(mapped) ? mapped : undefined
   ...
   */
  let sessionSink: DshSessionStateHandle | undefined
  const unmappedSessions = new Set<string>()
  /**
   * 探测失败的原因。用 `Set` 而不是数组：这个诊断每轮都会被读一次，
   * 累加的数组会让诊断文本每次都变（同一个错误重复出现），
   * 既刷屏、又让"结论变化才报告"的去重完全失效。
   */
  const discoveryErrors = new Set<string>()
  /**
   * 激活后填上，供诊断读取"哪些成员还没起来"。
   * 用持有变量而不是直接闭包引用 `host`：`extraDiagnostics` 要在 `host` 之前交给宿主，
   * 而它只在升级失败时才被调用（那时 `host` 必然已就绪）。
   */
  let hostRef: { readonly pendingMembers: readonly string[] } | undefined
  /**
   * 成员会话重探的间接引用。
   *
   ...
   */
  let refreshSessions: () => Promise<void> = async () => undefined
  const extraDiagnostics = (): readonly string[] => {
    const notes: string[] = []
    ...
    if (pending.length > 0) {
      notes.push(
        `成员 ${pending.join('、')} 尚未启动：启动成员需要上游的**活 Agent** 作授权凭据，` +
          'dsh 刚启动、用户还没开任何会话时会失败。这些成员无法接收摘要投递；' +
          'pulse() 每轮会自动重试。',
      )
    }
    for (const observation of sessionSink?.observations ?? []) {
      if (observation.memberId === undefined) unmappedSessions.add(observation.sessionId)
    }
    if (unmappedSessions.size > 0) {
      notes.push(
        `观察到 ${unmappedSessions.size} 个未映射的上游会话 id（例如 ${[...unmappedSessions].slice(0, 3).join('、')}）：` +
    ...
    if (sessionIndex.size === 0) {
      notes.push(
        '当前没有任何"会话 → 成员"映射：成员状态只能靠上游事件驱动，而事件也归属不到成员。' +
  ...
  const persisted: { rows: readonly unknown[]; at: number } = { rows: [], at: 0 }
  let persistedRefreshing = false
  const refreshPersistedSessions = (): void => {
    if (persistedRefreshing) return
    const controller = readService(ctx, 'sessionController') as
  ...
  const persistedRows = (): readonly unknown[] => {
    if (Date.now() - persisted.at > PERSISTED_TTL_MS) refreshPersistedSessions()
  ...
  const host = await activateMeetingHost({
    ...
    resumeWork: (sessionId, text) => {
      const agents = readService(ctx, 'agents') as
        | { get?: (id: string) => { inbox?: { append?: (target: string, message: unknown) => void } } | undefined }
        | undefined
      const agent = agents?.get?.(sessionId)
      const inbox = agent?.inbox
      if (inbox === undefined || typeof inbox.append !== 'function') return
  ...
  const refreshMemberSessions = async (): Promise<void> => {
    if (typeof runtime.memberSessions !== 'function') return
    try {
      for (const { slot, sessionId } of await runtime.memberSessions()) {
        if (sessionId.length > 0 && resolveMemberId(slot) !== undefined) sessionIndex.set(sessionId, slot)
      }
    } catch (error) {
  ...
  const applyActivity = (input: { sessionId: string; memberId: string | undefined; kind: SessionActivityKind }): void => {
    // 先喂活动跟踪：**按会话 id 记**，因为候选列表要对全部会话（含非成员）判断忙/闲。
    if (input.kind === 'busy') activity.markBusy(input.sessionId)
    else activity.markIdle(input.sessionId)

    const memberId = input.memberId
    if (memberId === undefined) return // 非成员：只更新忙/闲，不碰状态机。

    let member: AgentParticipant
    try {
    ...
    if (input.kind === 'busy') {
      ...
      if (member.state === 'idle-waiting' || member.state === 'done') member.resume()
      return
    }
   ...
   * 那又是"什么都没发生"的另一种写法。
   */
  let lastPulseNote: string | undefined
  const logger = resolveLogger(ctx)
  const reportPulse = (result: MeetingPulseResult): void => {
    ...
    if (note === undefined || note === lastPulseNote) return
    lastPulseNote = note

    try {
      if (escalation.escalated) logger?.info?.(`[dsh-meeting] ${note}`)
      else logger?.warn?.(`[dsh-meeting] ${note}`)
    } catch {
      // 宿主日志本身出问题不能影响轮次。
    }
    try {
