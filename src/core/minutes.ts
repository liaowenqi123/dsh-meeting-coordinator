 ...
 * 这四条是判据：命中任意一条才算相关。
 */
export const RELEVANCE_RULES: readonly string[] = [
...
export function validateMinutes(input: {
  ...
  readonly maxChars: number
}): { readonly ok: true; readonly chars: number } | { readonly ok: false; readonly reason: string } {
  ...
  if (input.transcriptChars > 0 && chars >= input.transcriptChars) {
    return {
      ok: false,
      reason:
        `${input.candidate.participant} 的纪要（${chars} 字）不短于会议记录本身（${input.transcriptChars} 字），` +
