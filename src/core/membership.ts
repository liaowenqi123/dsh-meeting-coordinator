...
export type Admission =
  | { readonly ok: true }
  | { readonly ok: false; readonly code: AdmissionCode; readonly reason: string }
 ...
 * 否则用户只会看到按钮灰着却不知道为什么。
 */
export function admitSession(input: AdmissionInput): Admission {
  const { candidate, roomId, currentRoomId } = input
  const operation: MembershipOperation = input.operation ?? 'join'
  ...
  if (candidate.isSubagent) {
    return {
      ...
      code: 'subagent',
      reason:
        `会话 ${candidate.sessionId} 是别的会话派生出来的子 Agent，不享有进入会议室的能力。` +
        '只有顶级会话才能作为成员。',
    }
  }
  if (operation === 'leave') {
    ...
    if (currentRoomId !== undefined) {
      return {
        ...
        reason:
          `会话 ${candidate.sessionId} 已经加入了会议室 ${currentRoomId}；` +
...
export function partitionCandidates(input: {
  ...
  const admittable: SessionCandidate[] = []
  const rejected: { candidate: SessionCandidate; reason: string; code: AdmissionCode }[] = []
  for (const candidate of input.candidates) {
    ...
    if (verdict.ok) admittable.push(candidate)
    else rejected.push({ candidate, reason: verdict.reason, code: verdict.code })
  }
