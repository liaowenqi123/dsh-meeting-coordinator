...
import { AgentParticipant, HUMAN_PARTICIPANT, type AgentParticipantSpec } from './participant.js'
import {
  DEFAULT_ROOM_POLICY,
  MeetingRoom,
...
export interface RoomMeetingCall {
  ...
  readonly humanInput?: readonly { readonly afterRound: number; readonly text: string }[] | undefined
  /** 会议进行中的观察与驱动钩子。 */
...
export class MeetingOrchestrator {
  private readonly members = new Map<string, AgentParticipant>()
  private readonly registry: RoomRegistry
  private readonly voice: MeetingVoicePort
  private readonly moderator: ModeratorPort
  private readonly policy: MeetingRoomPolicy
  private readonly minutesMaxChars: number
  private readonly contextOf: ((sessionId: string, maxChars: number) => string | undefined) | undefined
  private readonly minutes: RoomMinutesSink | undefined
  private readonly agentOf: ((sessionId: string) => unknown) | undefined
  private readonly stopRequested: (() => boolean) | undefined
  private readonly resumeWork: ((sessionId: string, text: string) => void | Promise<void>) | undefined
  private readonly now: () => number
  private counter = 0
  private active: MeetingRoom | undefined
  /** 进行中会议所在的**会议室** id（会籍实体）。散会时清空。 */
  private activeRoomId: string | undefined
  private activeTitles: Record<string, { title: string; domain: string }> = {}
  private readonly history: RoomMeetingRecord[] = []
  ...
  member(id: string): AgentParticipant {
    ...
    if (found === undefined) throw new MeetingOrchestratorError(`未登记的会话 ${id}。`)
    return found
  }
   ...
   */
  private projectionFor(id: string): string {
    ...
    if (borrowed === undefined || borrowed.trim().length === 0) return own
  ...
  forget(sessionId: string): 'ok' | 'busy' | 'absent' {
    ...
    if (member.isInMeeting() || member.isAwaitingEntry()) return 'busy'
  ...
  states(): Readonly<Record<string, string>> {
    ...
    for (const id of this.participantIds) out[id] = this.member(id).state
    return out
  }
  ...
  onWorkUnitComplete(sessionId: string): boolean {
    ...
    if (!member.isInMeeting()) return false
    const room = this.active
    if (room === undefined) return false
    if (room.hasPresent(sessionId) || room.isClosed) return false
    const turn = room.admit(sessionId, {
      memoryProjection: (id) => this.projectionFor(id),
      titles: this.activeTitles,
    })
    if (turn !== undefined) this.admittedLate.push(sessionId)
    return turn !== undefined
  }

  private admittedLate: string[] = []
   ...
   */
  private meetingFailures: string[] = []

  /** 人在会中实时插话。不占轮次、不占发言名额。 */
  injectHumanTurn(text: string): MeetingTurn {
    const room = this.active
    if (room === undefined) throw new MeetingOrchestratorError('当前没有正在进行的会议，人类无法插话。')
    return room.appendHumanTurn(text)
  }

  // --- 主流程 -------------------------------------------------------------

  async convene(call: RoomMeetingCall): Promise<RoomMeetingRecord> {
    ...
    if (call.calledBy !== HUMAN_PARTICIPANT && !this.registry.isMember(call.calledBy)) {
      throw new MeetingOrchestratorError(
        `会话 ${call.calledBy} 不是会议室 ${call.roomId} 的成员，没有召集权。` +
          '只有加入了会议室的会话才能唤起会议。',
      )
    }
    ...
    for (const id of invited) {
      ...
      if (!member.canBeSummoned()) continue // 已在会中或已在等入场
      member.summon()
      summoned.push(id)
      if (member.isAwaitingEntry()) deferred.push(id)
    }
    ...
    if (present.length === 0) {
      ...
      for (const id of summoned) this.member(id).cancelSummon()
      throw new MeetingOrchestratorError(
        `全部 ${deferred.length} 位成员都在工作中（${deferred.join('、')}），会议无法开始。` +
    ...
    const notes: { participant: string; text: string; chars: number }[] = []
    let adjournedReason = ''
    ...
    try {
      ...
      for (const id of room.present.filter((member) => member !== HUMAN_PARTICIPANT)) {
        ...
        try {
          ...
          if (!verdict.ok) throw new MeetingOrchestratorError(`会后压缩被拒绝：${verdict.reason}`)
    ...
    const spokenRounds = room.transcript
      ...
      .map((turn) => turn.round)
    const record: RoomMeetingRecord = {
      room,
      calledBy: call.calledBy,
      summoned,
      deferred,
   ...
   * @returns 是否发言成功。
   */
  private async speakSafely(room: MeetingRoom, sessionId: string): Promise<boolean> {
    try {
   ...
   */
  private async runDiscussion(room: MeetingRoom, call: RoomMeetingCall): Promise<string> {
    const humanAfter = (round: number): void => {
      for (const utterance of call.humanInput ?? []) {
        if (utterance.afterRound === round) room.appendHumanTurn(utterance.text)
      }
    }
   ...
   * 就留在本轮，由主持人点名补上。
   */
  private async advanceIfRoundComplete(room: MeetingRoom, call: RoomMeetingCall): Promise<boolean> {
    const presentAgents = room.present.filter((id) => id !== HUMAN_PARTICIPANT)
    if (presentAgents.length === 0) return false
    // 用"已处理过"而不是"发言过"：本轮发言失败的人也算处理过了，
    // 否则一场网络抖动会让本轮永远推进不下去。
    const settled = new Set(room.settledThisRound())
    if (!presentAgents.every((id) => settled.has(id))) return false

    const finishedRound = room.round
    await call.hooks?.onRoundEnd?.(finishedRound, room)
    for (const utterance of call.humanInput ?? []) {
      if (utterance.afterRound === finishedRound) room.appendHumanTurn(utterance.text)
    }
    room.nextRound()
    return true
  }
  ...
  private async speak(room: MeetingRoom, sessionId: string): Promise<void> {
    const member = this.member(sessionId)
  ...
  private callerModel(caller: string): string | undefined {
    if (caller === HUMAN_PARTICIPANT) return undefined
 ...
 */
function resumePrompt(title: string, reason: string, note: string): string {
  return [
