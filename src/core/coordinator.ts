...
import { evaluateTriggers, type TriggerDecision, type TriggerEvaluation, type TriggerPolicy } from './triggers.js'
...
import type { AgentHandle, AgentRuntimePort, DeliveryMode, RuntimeCapabilities } from '../ports/agent-runtime.js'
...
export interface MeetingRecord {
  ...
  readonly failures: readonly { readonly slot: AgentSlotId; readonly error: string }[]
}
...
export const DEFAULT_COORDINATOR_POLICY: TriggerPolicyPolicy = {
...
export type CallMeetingResult =
  | { readonly accepted: true; readonly meeting: MeetingRecord }
  | { readonly accepted: false; readonly reason: string }

export class MeetingCoordinator {
  private readonly board: BriefingBoard
  private readonly runtime: AgentRuntimePort
  private readonly policy: TriggerPolicyPolicy
  private readonly now: () => number
  private readonly specs = new Map<AgentSlotId, AgentSlotSpec>()
  private readonly handles = new Map<AgentSlotId, AgentHandle>()
  private readonly lastCallAt = new Map<AgentSlotId, number>()
  private lastMeetingAt: number | undefined
  private lastMeetingRound: number | undefined
  ...
  get slotSpecs(): readonly AgentSlotSpec[] {
    return this.slotIds.map((id) => {
      ...
      if (spec === undefined) throw new RuntimeUnavailable(`槽位 ${id} 规格丢失。`)
      return spec
    })
  }
   ...
   */
  async start(options?: { readonly signal?: AbortSignal | undefined }): Promise<readonly AgentHandle[]> {
    const capabilities = this.runtime.capabilities()
    if (!capabilities.canSpawn) {
      throw new RuntimeUnavailable(
        `运行时 ${capabilities.port} 不具备 spawn 能力，无法启动领域隔离子 Agent。` +
          `诊断：${capabilities.notes.join(' / ') || '上游未提供说明'}`,
      )
    }
    const started: AgentHandle[] = []
   ...
   */
  async callMeeting(call: MeetingCall): Promise<CallMeetingResult> {
  ...
  async convene(
    ...
    stallSignals: readonly StallSignal[] = [],
    origin?: {
    ...
    const deliveredTo: AgentSlotId[] = []
    const woke: AgentSlotId[] = []
    const failures: { slot: AgentSlotId; error: string }[] = []

    // 摘要必须**唤起**接收者，而不是留给它下次自己发现。
    // 这是"互相唤起"的最后一段：会议不是公告栏，是点名让人动。
    const mode: DeliveryMode = 'wake'
  ...
  private selectParticipants(
    decision: TriggerDecision,
    stallSignals: readonly StallSignal[],
    invitees: readonly AgentSlotId[] | undefined,
  ): readonly AgentSlotId[] {
    ...
    if (decision.scope === 'global') return this.slotIds

    const chosen = new Set<AgentSlotId>()
    ...
    if (chosen.size === 0) return this.slotIds
   ...
   */
  wakeGraph(): readonly { readonly from: string; readonly to: string; readonly by: AgentSlotId | null }[] {
    const edges: { from: string; to: string; by: AgentSlotId | null }[] = []
  ...
  private persistMeeting(record: MeetingRecord): void {
