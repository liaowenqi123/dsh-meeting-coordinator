...
export const HUMAN_PARTICIPANT = 'human' as const
...
export class AgentParticipant {
  ...
  private _state: ParticipantState = 'working'
  /**
   ...
   */
  private stateBeforeSummon: ParticipantState | undefined = undefined
  private readonly context: string[] = []
  private readonly notes: MeetingNote[] = []
  ...
  summon(): void {
    if (!this.canBeSummoned()) {
      throw new ParticipantStateError(
        `会话 ${this.id} 当前状态 ${this._state} 不可被召集（已在会中或已待入场）。`,
      )
    }
  ...
  onWorkUnitComplete(): void {
    if (this._state !== 'awaiting-entry') return
    this._state = 'in-meeting'
  }

  /** 取消召集（例如会议在它入场前就散了，或人类喊停）。 */
  cancelSummon(): void {
    if (this._state === 'awaiting-entry') this.restoreAfterSummon()
  }
   ...
   */
  private restoreAfterSummon(): void {
    const previous = this.stateBeforeSummon
    this.stateBeforeSummon = undefined
    this._state = previous ?? 'idle-waiting'
  }

  private transition(to: ParticipantState, from: readonly ParticipantState[], what: string): void {
    if (!from.includes(this._state)) {
      throw new ParticipantStateError(
        `参与者 ${this.id} 无法${what}：当前状态 ${this._state}，允许的来源状态为 ${from.join('/')}。`,
      )
    }
    this._state = to
  }
  ...
  memoryProjection(maxChars: number): string {
    ...
    if (this.context.length > 0) {
      lines.push('你的工作记忆：')
      for (const line of this.context) lines.push(`- ${line}`)
    }
    if (this.notes.length > 0) {
      lines.push('你历次开会后记下的、与你自己相关的要点：')
      for (const note of this.notes) lines.push(`- [${note.meetingId}] ${note.text}`)
    }
