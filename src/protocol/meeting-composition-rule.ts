...
import { BRIEFING_BOARD_KIND, MEETING_API_VERSION } from './meeting-protocol.js'

export const briefingBoardCompositionRule: ProtocolCompositionRule = {
  apiVersion: MEETING_API_VERSION,
  kind: BRIEFING_BOARD_KIND,

  preflight(input) {
    const issues: {
      readonly code: 'protocol-preflight-failed'
      readonly severity: 'error'
      readonly path: string
      readonly message: string
    }[] = []
    const bindings: { readonly requirementId: string; readonly supportIds: readonly string[] }[] = []

    for (const requirement of input.requirements) {
      // `compose` 已经按 protocol definition 过滤过 potentialSupports，这里直接用。
      const candidates = input.potentialSupports
      if (candidates.length === 0) {
        // 缺候选 support 已由 compose 报 `potential-support-missing`，不重复报。
        continue
      }

      const rawSpec = requirement.requirement.spec
      const requestedDomain =
        typeof rawSpec === 'object' && rawSpec !== null && !Array.isArray(rawSpec)
          ? (rawSpec as { boardDomain?: unknown }).boardDomain
          : undefined

      if (requestedDomain === undefined) {
        const domains = new Set(
          candidates
            .map((candidate) => (candidate.support.spec as { boardDomain?: unknown } | undefined)?.boardDomain)
            .filter((value): value is string => typeof value === 'string'),
        )
        if (domains.size > 1) {
          issues.push({
            code: 'protocol-preflight-failed',
            severity: 'error',
            path: `/requirements/${requirement.id}`,
            message:
              `消费方未指定 boardDomain，但候选 coordinator 分属 ${domains.size} 块板 ` +
              `(${[...domains].sort().join(', ')})。请在消费方 spec 中显式给出 boardDomain，` +
              '或在协商 policy 中指定 selectCoordinator。',
          })
          continue
        }
      }
      ...
      bindings.push({
        ...
        supportIds: candidates.map((candidate) => candidate.id),
      })
    }
